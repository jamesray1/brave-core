...
int GetDataVersion(PrefService* prefs) {
  ...
  // Check if returned version was from preferences override.
  if (!prefs || !prefs->HasPrefPath(prefs::kSearchProviderOverridesVersion))
    dataVersion += kBraveCurrentDataVersion;
  return dataVersion;
}

// Redefines function with the same name in Chromium. Modifies the function to
// get the default search for country id istead of always using index 0 for the
// default search.
